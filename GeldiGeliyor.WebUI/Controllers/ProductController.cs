...
{
    public class ProductController(IProductService productService,IMemoryCache cache,ICategoryService categoryService) : Controller
    {
        [Authorize(Roles = "Customer,Seller")]
        public async Task<IActionResult> Index()
        {
            //List<ProductGetDto> productGetDto = await productService.ProductsWithPicturesAndSellerAsync();
            List<ProductGetDto> productGetDtos = await cache.GetOrCreateAsync<List<ProductGetDto>>("products", cacheEntry =>
            {
                cacheEntry.AbsoluteExpirationRelativeToNow = TimeSpan.FromMinutes(1);
                return productService.ProductsWithPicturesAndSellerAsync();
            });

            return View(productGetDtos);
            //throw new Exception("Ben gonderdim");
        }
